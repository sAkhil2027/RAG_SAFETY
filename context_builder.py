...
# # Load .env (project root) for model name used in token counting
# load_dotenv(dotenv_path=os.path.join(os.path.dirname(__file__), "..", ".env"))
# MODEL_NAME = os.getenv("GROQ_MODEL", "groq/compound-mini")

# def _count_tokens(text: str) -> int:
#     """Return an approximate token count for *text*.
#     Uses `tiktoken` if available; otherwise falls back to a 4‑chars‑per‑token heuristic.
#     """
#     try:
#         import tiktoken
...
#     except Exception:
#         # Rough estimate – ensure at least one token
...
#     The function walks the list of *chunks* (each a dict with a ``content`` key),
#     accumulating tokens until the limit is reached. The last chunk may be sliced
#     to fit the remaining token budget.
#     """
#     context_parts = []
#     total_tokens = 0
#     for chunk in chunks:
...
#             if remaining <= 0:
#                 break
#             # Approximate character count for the remaining tokens
